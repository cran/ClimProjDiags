...
#'
#'@param data A numeric n-dimensional array containing daily maximum or minimum temperature 
#'@param threshold An array with the threshold to be/not to be reach, usually given by the 90th/10th percentiles for heat/cold waves computed with the \code{Threshold} function.
#'@param op A character ">" (by default) or ">=" for heat waves and "<" or "<=" for cold waves indicating the operator must be used  to compare data to threshold.
#'@param spell.length A number indicating the number of consecutive days with extreme temperature to be considered heat or cold wave.
#'@param by.seasons If TRUE (by default), the wave duration is computed for each season (DJF/MAM/JJA/SON) separately. If FALSE is specified, the monthly wave duration is computed.
#'@param dates A vector of dates including calendar attributes. If NULL (by default), the 'time' attributes of parameter 'data' is used.
...
#'str(a)
#'@export
WaveDuration <- function(data, threshold, op = ">", spell.length = 6, by.seasons = TRUE, dates = NULL, 
  ...
  if (length(dim_names) > 1) {
    ...
    result <- list()
    result$output1 <- .WaveDuration(data = data[[1]], threshold = data[[2]], date.factor = date.factor, 
